...
|---|---|---|---|---|---|
| `BUG-CI-001` | `.github/workflows/build.yml` used `continue-on-error: true` for lint, allowing lint errors to leave CI green. | `45e72840eec5019b5fe0e0a20ed2946a8628929e` | Local working-tree change removes `continue-on-error` and keeps `warningsAsErrors=false` in `app/build.gradle.kts`. | Local diff, `git diff --check`, and Prettier workflow check passed. GitHub Actions verification is pending. | **Fixed locally; CI verification pending** |
